...
largest.in.peaks[, channel.problem := paste0(channel, problem.i)]
prob.data[, channel.problem := paste0(channel, problem.i)]
single.l[, channel.problem := paste0(channel, problem.i)]
single.l.wide[, channel.problem := paste0(channel, problem.i)]
sample.colors <- c(
  mixture="black",
  ## one.part="#A6CEE3", #blues
  ## four.parts="#1F78B4",
...
viz <- animint(
  ...
  scatter=ggplot()+
    ggtitle("Relative height comparison, select peak")+
    coord_equal()+
    theme_bw()+
    geom_abline(slope=1, intercept=0, color="grey")+
    geom_point(aes(
    ...
    scale_y_continuous(
      "single source peak relative height"),
  peaks=ggplot()+
    ggtitle("Three samples for selected peak")+
    theme_bw()+
    theme(panel.margin=grid::unit(0, "lines"))+
    scale_color_manual(values=sample.colors)+
    coord_cartesian(xlim=c(-3, 3))+
    geom_line(aes(
      bases.from.middle, relative.height, color=Sample, group=Sample),
      size=1, 
    ...
    geom_point(aes(
      ...
      showSelected="channel.problem",
      data=largest.in.peaks))
animint2dir(viz, "figure-one-pair-models-joint-viz", open.browser=FALSE)
